...
A static PNG is saved to data/review/tfidf_lsa_tsne.png.
...
N_PCA = 50
print(f"Running LSA ({N_PCA} components) on TF-IDF...")
svd = TruncatedSVD(n_components=N_PCA, random_state=42)
pca_coords = svd.fit_transform(_tfidf)
print(f"LSA explained variance: {svd.explained_variance_ratio_.sum():.1%}")

print("Running t-SNE on LSA-50 space (this takes ~10-20 s)...")
reducer = TSNE(
    n_components=2,
    perplexity=30,
    learning_rate="auto",
...
# ── save & open ───────────────────────────────────────────────────────────────
out_png = os.path.join(config._PROJECT_DIR, "data", "review", "tfidf_lsa_tsne.png")
