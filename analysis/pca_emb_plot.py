...
directly from the document text and then plots PCA directly on it — so the layout reflects
...
A static PNG is saved to data/review/pca_emb.png.
...
# ── Embeddings → PCA(2D direct) ──────────────────────────────────────────────
print("Running PCA on embeddings (plotting PC1 vs PC2 directly)...")
pca = PCA(n_components=2, random_state=42)
coords = pca.fit_transform(embeddings)
var1, var2 = pca.explained_variance_ratio_
print(f"PC1: {var1:.1%}  PC2: {var2:.1%}  combined: {var1+var2:.1%}")
...
# Correlate TF-IDF term weights with PC1/PC2 coordinates.
print("Computing Spearman correlations with PCA axes...")
...
# ── colorblind-friendly palette overrides ────────────────────────────────────
...
fig.text(0.5, 0.91,
         f"all-MiniLM-L6-v2  \u2192  PCA(2D)   \u2502   {len(metadatas)} job descriptions   (PC1 {var1:.1%} var, PC2 {var2:.1%} var)",
...
def scatter_on(ax, mode, panel_title):
    ...
    ax.set_xlabel(f"PC 1 ({var1:.1%} var)", fontsize=12, labelpad=6)
    ax.set_ylabel(f"PC 2 ({var2:.1%} var)", fontsize=12, labelpad=6)
    palette = PALETTES[mode]
    leg = ax.legend(
...
# ── save & open ───────────────────────────────────────────────────────────────
out_png = os.path.join(config._PROJECT_DIR, "data", "review", "pca_emb.png")
